# -*- coding: utf-8 -*-
"""Cuadro&Gra_173577.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xrH3gaHB6LDF5_qgKqrTbth-shXhYF1F
...
# Datos de porcentajes de juegos ganados por división
afc_east = [0.688, 0.688, 0.563, 0.250, 0.125]
afc_central = [0.688, 0.625, 0.438, 0.313]
afc_west = [0.688, 0.625, 0.500, 0.438, 0.125]
nfc_east = [0.813, 0.688, 0.563, 0.375, 0.250]
nfc_central = [0.688, 0.563, 0.313, 0.313, 0.313]
nfc_west = [0.875, 0.750, 0.375, 0.375]

# a combinar estadísticas de todos los porcentajes de los juegos ganados
todos_porcentajes=afc_east+afc_central+afc_west+nfc_east+nfc_central+nfc_west
todos_porcentajes.sort()  # Ordenar los datos

#clasifique los datos en cinco clases de igual tamaño, mutuamente excluyentes
n_datos = len(todos_porcentajes)
tamano_clase = n_datos / 5
limites = []
for i in range(5):
    indice = int(i * tamano_clase)
    if i == 0:
        limite_inferior = min(todos_porcentajes)
    else:
        limite_inferior = limites[-1][1]

    if i == 4:
        limite_superior = max(todos_porcentajes)
    else:
        limite_superior = todos_porcentajes[int((i + 1) * tamano_clase)]
...
for limite in limites:
    freq_abs = sum(1 for x in todos_porcentajes if limite[0] <= x <= limite[1])
    freq_rel = freq_abs / n_datos
    frecuencias.append({
        'Límite inferior': limite[0],
        'Límite superior': limite[1],
...
print("------------------------------------------------------------------")
print("Clase |       Límites     | Frec. Absoluta | Frec. Relativa")
print("-----------------------------------------------------------------")
for i, f in enumerate(frecuencias, 1):
    print(f"{i}\t [{f['Límite inferior']:.3f}, {f['Límite superior']:.3f}] \t {f['Frecuencia absoluta']} \t\t {f['Frecuencia relativa']:.3f}")

# c) Polígono de frecuencias
plt.figure(figsize=(10, 6))
puntos_medios = [(f['Límite inferior'] + f['Límite superior'])/2 for f in frecuencias]
frecuencias_abs = [f['Frecuencia absoluta'] for f in frecuencias]

plt.plot(puntos_medios, frecuencias_abs, 'o-')
plt.title('Polígono de Frecuencias - Porcentajes de Juegos Ganados')
...
plt.figure(figsize=(10, 6))
frecuencias_acum = []
suma_actual = sum(f['Frecuencia absoluta'] for f in frecuencias)
for f in frecuencias:
    frecuencias_acum.append(suma_actual)
    suma_actual -= f['Frecuencia absoluta']

plt.plot(puntos_medios, frecuencias_acum, 'o-')
plt.title('Ojiva Mayor Que - Porcentajes de Juegos Ganados')
plt.xlabel('Porcentaje de Juegos Ganados')
...
print(f"Media: {np.mean(todos_porcentajes):.3f}")
print(f"Mediana: {np.median(todos_porcentajes):.3f}")
print(f"Desviación Estándar: {np.std(todos_porcentajes):.3f}")
